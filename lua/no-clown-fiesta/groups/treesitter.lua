local M = {}

function M.highlight(palette, opts)
    return {
        ['@annotation'] = { fg = palette.white },
        ['@attribute'] = { fg = palette.white },
        -- ['@boolean'] = { fg = palette.purple_light },
        ['@boolean'] = { fg = palette.gray_blue },
        ['@character'] = { fg = palette.green },
        ['@comment'] = { link = 'Comment' },
        ['@comment.documentation'] = { fg = palette.medium_gray },
        ['@comment.error'] = { fg = palette.error },
        ['@comment.note'] = { fg = palette.light_gray },
        ['@comment.todo'] = { fg = palette.todo },
        ['@comment.warning'] = { fg = palette.warning },
        ['@const.builtin'] = { fg = palette.red },
        ['@const.macro'] = { fg = palette.cyan },
        ['@constant'] = { fg = palette.white },
        ['@constructor'] = { fg = palette.cyan },
        ['@constructor.lua'] = { link = '@punctuation.bracket' },
        ['@diff.delta'] = {
            fg = palette.gray_blue,
            bg = palette.accent_blue,
            bold = true,
        },
        ['@error'] = { fg = palette.error },
        -- ['@function'] = vim.tbl_extend('force', { fg = palette.cyan }, opts.styles.functions),
        -- ['@function.method'] = { fg = palette.cyan },
        -- ['@function.method.call'] = { fg = palette.cyan },
        ['@function'] = vim.tbl_extend('force', { fg = palette.orange_light }, opts.styles.functions),
        ['@function.method'] = { fg = palette.orange_light },
        ['@function.method.call'] = { fg = palette.orange_light },
        ['@function.builtin'] = { fg = palette.cyan },
        ['@function.macro'] = { fg = palette.cyan },
        ['@keyword'] = vim.tbl_extend('force', { fg = palette.gray_blue }, opts.styles.keywords),
        -- ['@keyword.conditional'] = { fg = palette.gray_blue },
        -- ['@keyword.repeat'] = { fg = palette.gray_blue },
        ['@keyword.conditional'] = { fg = palette.purple_light },
        ['@keyword.repeat'] = { fg = palette.purple_light },
        ['@keyword.return'] = { fg = palette.purple_light },
        ['@keyword.coroutine'] = { fg = palette.gray_blue },
        -- ['@keyword.exception'] = { fg = palette.red },
        ['@keyword.exception'] = { fg = palette.gray_blue },
        ['@keyword.function'] = vim.tbl_extend('force', { fg = palette.gray_blue }, opts.styles.keywords),
        ['@keyword.import'] = { fg = palette.purple_light },
        ['@keyword.export'] = { fg = palette.purple_light },
        -- ['@keyword.import'] = { fg = palette.red },
        ['@keyword.operator'] = { fg = palette.gray_blue },
        ['@label'] = { fg = palette.white },
        ['@lsp.type.comment'] = {},
        ['@markup'] = { fg = palette.fg },
        ['@markup.italic'] = { italic = true },
        ['@markup.link'] = { fg = palette.medium_gray_blue },
        ['@markup.list'] = { fg = palette.fg },
        ['@markup.list.unchecked'] = { fg = palette.fg },
        ['@markup.literal'] = { fg = palette.medium_gray },
        ['@markup.strong'] = { fg = palette.medium_gray },
        ['@markup.title'] = { fg = palette.medium_gray },
        ['@markup.underline'] = { underline = true },
        ['@module'] = { fg = palette.white },
        -- ['@number'] = { fg = palette.purple_light },
        -- ['@number.float'] = { fg = palette.purple_light },
        ['@number'] = { fg = palette.gray_blue },
        ['@number.float'] = { fg = palette.gray_blue },
        ['@operator'] = { fg = palette.medium_gray },
        ['@property'] = { fg = palette.white },
        ['@punctuation.bracket'] = { fg = palette.medium_gray },
        ['@punctuation.delimiter'] = { fg = palette.medium_gray },
        ['@string'] = { fg = palette.medium_gray_blue },
        -- ['@string'] = { fg = '#AD9E80' },
        ['@string.escape'] = { fg = palette.medium_gray_blue },
        ['@string.regexp'] = { fg = palette.medium_gray_blue },
        ['@string.special.path'] = { fg = palette.light_gray },
        ['@string.special.symbol'] = { fg = palette.medium_gray },
        ['@tag'] = { fg = palette.gray_blue },
        -- ['@tag.attribute'] = { fg = palette.light_gray },
        ['@tag.attribute'] = { fg = palette.cyan },
        ['@tag.delimiter'] = { fg = palette.medium_gray },
        ['@text.uri'] = { fg = palette.light_gray },
        ['@type'] = vim.tbl_extend('force', { fg = palette.cyan }, opts.styles.type),
        -- ['@type'] = vim.tbl_extend('force', { fg = palette.orange_light }, opts.styles.type),
        ['@type.builtin'] = { fg = palette.cyan },
        ['@variable'] = vim.tbl_extend('force', { fg = palette.white }, opts.styles.variables),
        ['@variable.builtin'] = { fg = palette.gray_blue },
        ['@variable.field'] = { fg = palette.white },
        ['@variable.parameter'] = { fg = palette.white },
        ['@variable.parameter.reference'] = { fg = palette.white },
    }
end

return M
